...
public class CustomHlsPlaylistParser implements ParsingLoadable.Parser<HlsPlaylist> {
  ...
  private static CustomHlsMasterPlaylist parseMasterPlaylist(CustomHlsPlaylistParser.LineIterator iterator, String baseUri)
    ...
    ArrayList<CustomHlsMasterPlaylist.HlsUrl> variants = new ArrayList<>();
    ArrayList<CustomHlsMasterPlaylist.HlsUrl> audios = new ArrayList<>();
    ArrayList<CustomHlsMasterPlaylist.HlsUrl> subtitles = new ArrayList<>();
        ...
        if (variantUrls.add(line)) {
          Format format =
              Format.createVideoContainerFormat(
                  ...
                  /* selectionFlags= */ 0);
          variants.add(new CustomHlsMasterPlaylist.HlsUrl(line, format));
        }
      }
    }
      ...
      switch (parseStringAttr(line, REGEX_TYPE, variableDefinitions)) {
        case TYPE_AUDIO:
          ...
          if (isMediaTagMuxed(variants, uri)) {
            ...
            audios.add(new CustomHlsMasterPlaylist.HlsUrl(uri, format));
          }
          break;
        case TYPE_SUBTITLES:
          format =
              Format.createTextContainerFormat(
                  ...
                  selectionFlags,
                  language);
          subtitles.add(new CustomHlsMasterPlaylist.HlsUrl(uri, format));
          break;
      ...
      if (line.startsWith(TAG_PLAYLIST_TYPE)) {
        ...
        segments.add(
            new HlsMediaPlaylist.Segment(
                replaceVariableReferences(URLEncoder.encode(line, "utf-8"), variableDefinitions),
  ...
  DrmInitData.SchemeData parseWidevineSchemeData(
    ...
    if (KEYFORMAT_WIDEVINE_PSSH_JSON.equals(keyFormat)) {
      try {
        return new DrmInitData.SchemeData(C.WIDEVINE_UUID, "hls", line.getBytes(C.UTF8_NAME));
  ...
  private static int parseIntAttr(String line, Pattern pattern) throws ParserException {
    return Integer.parseInt(parseStringAttr(line, pattern, Collections.<String, String>emptyMap()));
  }

  private static long parseLongAttr(String line, Pattern pattern) throws ParserException {
    return Long.parseLong(parseStringAttr(line, pattern, Collections.<String, String>emptyMap()));
  }

  private static double parseDoubleAttr(String line, Pattern pattern) throws ParserException {
    return Double.parseDouble(parseStringAttr(line, pattern, Collections.<String, String>emptyMap()));
  }
  ...
  private static boolean isMediaTagMuxed(
      List<CustomHlsMasterPlaylist.HlsUrl> variants, String mediaTagUri) {
    if (mediaTagUri == null) {
      return true;
    }
